...
function getBookings() {
    return JSON.parse(localStorage.getItem('jetride_bookings')) || [];
}

function saveBookings(bookings) {
    localStorage.setItem('jetride_bookings', JSON.stringify(bookings));
}
...
function getFavorites() {
    return JSON.parse(localStorage.getItem('jetride_favorites')) || [];
}

function saveFavorites(favorites) {
    localStorage.setItem('jetride_favorites', JSON.stringify(favorites));
}
            ...
            <div class="tour-card" data-id="${tour.id}">
                ...
                <div class="card-content">
                    ...
                    <div class="card-actions">
                        ...
                        <button class="favorite-btn ${isFavorite ? 'active' : ''}" data-id="${tour.id}">
...
function attachCardEvents() {
    ...
    document.querySelectorAll('.tour-card').forEach(card => {
        card.addEventListener('click', (e) => {
            ...
            if (e.target.closest('.view-tour-btn')) return;
...
function openModal(tour) {
    ...
    favBtn.onclick = function() {
        ...
        const nowFavorite = newFavorites.includes(tour.id);
        this.textContent = nowFavorite ? '❤️ В избранном' : '🤍 В избранное';
        this.classList.toggle('active', nowFavorite);
        // Обновляем иконку на карточке (если она видна)
        const cardBtn = document.querySelector(`.favorite-btn[data-id="${tour.id}"]`);
        if (cardBtn) {
            cardBtn.classList.toggle('active', nowFavorite);
            cardBtn.innerHTML = `<i class="${nowFavorite ? 'fas' : 'far'} fa-heart"></i>`;
        }
    };
